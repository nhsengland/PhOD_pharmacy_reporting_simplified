---
title: "Unplanned Pharmacy Closures Monthly Pack - INTERNAL USE"
subtitle: "National"
...
#source("~/Rprojects/Unplanned-pharmacy-closures/R/import_unplanned_closures_data - UDAL.R")
#source("~/Rprojects/Unplanned-pharmacy-closures/R/monthly_pack_graphs.R")
 ...
 * [Reasons for Unplanned Closure by Organisation](#reason-for-closure)
...
* Out of the active pharmacies in England (excluding DACs), **`r get_perc_pharms_with_closure()`%** have reported at least one unplanned closure since October 2021 (the month data collection for closures began).

* From October 2021 to `r format(Sys.Date() - lubridate::weeks(5), '%B, %Y')`, the number of closures per pharmacy has been **`r get_100hr_to_40hr_closures_ratio()`** times higher for 100hr pharmacies than for 40hr pharmacies.
...
* `r format(Sys.Date() - lubridate::weeks(5), '%B, %Y')` has seen a **`r get_closure_number_change()`%** change in number of reported unplanned closures compared with `r format(Sys.Date() - lubridate::weeks(5), '%B')` last year.

* Up until July 2022, Lloyds had the highest number of unplanned closures per pharmacy across the country out of the large multiples. In July 2022 up to November 2022, Tesco had the highest number of unplanned closures per pharmacy. This month, **Lloyds** has reported the highest number of closures per pharmacy.
...
reactable(
  get_closures_by_STP_table(unplannedClosuresData, externalUse = FALSE),
  filterable = TRUE,
  #paginateSubRows = TRUE,
  minRows = 5,
  highlight = TRUE,
  height = 450,
  groupBy = c("Month of Closure","Region Name", "ICB Name", "Organisation"),
  columns = list(
...
data <- plot_total_duration_of_closures(unplannedClosuresData, plotChart = FALSE)

data %>%
  download_this(
    output_name = "duration_of_closures_by_stp",
    output_extension = ".xlsx",
    button_label = "Download data",
    button_type = "warning",
    has_icon = TRUE,
...
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.width=14, fig.height=5}
reactable(
  plot_total_duration_of_closures(unplannedClosuresData, plotChart = FALSE),
  filterable = TRUE,
  #paginateSubRows = TRUE,
  minRows = 5,
  highlight = TRUE,
  height = 450,
  groupBy = c("Month of closure", "Region Name", "STP Name"),
  columns = list(
    `Closure Duration (hours)`= colDef(aggregate = "sum", format = colFormat(digits = 0), filterable = FALSE)
  )
)
```
]
...
---
name: number-of-closures-org

### Number of Unplanned Closures By Organisation
...
.panel[.panel-name[National]
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.width=14, fig.height=5, fig.align='center'}
plot_closures_by_month_by_org(unplannedClosuresData,
                       level = "National",
                       region_STP_name = NULL,
                       per_100_pharms = FALSE)
```
...
name: number-of-closures-per-pharm-org

### Number of Unplanned Closures per 100 Pharmacies By Organisation
...
.panel[.panel-name[National]
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.width=14, fig.height=5, fig.align='center'}
plot_closures_by_month_by_org_per100pharm(unplannedClosuresData,
                       level = "National",
                       region_STP_name = NULL,
                       per_100_pharms = TRUE)
```
This graph shows the number of unplanned closures per 100 pharmacies in each organisation group so that the volumes are comparable.
...
---
name: duration-of-closures-org

### Total Duration of Unplanned Closures By Organisation
...
.panel[.panel-name[National]
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.width=14, fig.height=5, fig.align='center'}
plot_total_duration_of_closures_by_org(unplannedClosuresData)
```
...
name: perc-under-4hrs-org

### Percentage of closures under 4hrs by organisation
...
.panel[.panel-name[National]
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.width=14, fig.height=5, fig.align='center'}
plot_perc_closures_under_4hrs_by_org(data = unplannedClosuresData,
                                         level = "National",
                                         region_STP_name = NULL,
                                         plotChart = TRUE)
```
...
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.width=14, fig.height=5, fig.align='center'}
plot_perc_closures_under_4hrs_by_org(unplannedClosuresData,
                       level = "Regional",
                       region_STP_name = "South East")
```
...
### Reasons for Unplanned Closures by Organisation
...
.panel[.panel-name[National]
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.width=14, fig.height=5, fig.align='center'}
plot_reason_for_closure(Month = max(unplannedClosuresData$month_of_closure),
                        level = "National",
                        region_STP_name = NULL)
```
...
name: closures-per-pharm

### Unplanned closures per pharmacy by chain
...
.panel[.panel-name[National]
```{r echo = FALSE, warning = FALSE, message = FALSE, fig.width=14, fig.height=5, fig.align='center'}
plot_closures_per_pharm(Month = max(unplannedClosuresData$month_of_closure),
                        level = "National",
                        region_STP_name = NULL)
```
This graph shows the rate of closure for the top 10 chains with the highest closure rates.
